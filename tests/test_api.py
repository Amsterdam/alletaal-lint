...
class TestAPIEndpoints:
    ...
    def test_score_sentence_endpoint(self, client):
        ...
        response = client.post(
            "/score-sentence", json={"text": "De kat zit op de mat."}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_empty_text_handling(self, client):
        ...
        response = client.post("/score-sentence", json={"text": ""})
    ...
    def test_markdown_removal(self, client):
        ...
        response = client.post("/score-sentence", json={"text": markdown_text})
        assert response.status_code == 200
    ...
    def test_invalid_input_format(self, client):
        ...
        response = client.post("/score-sentence", json={"wrong_field": "test"})
        assert response.status_code == 422  # Validation error

    def test_very_long_text(self, client):
        """Test with very long text."""
        long_text = "Dit is een test. " * 1000
        response = client.post("/score-document", json={"text": long_text})
        assert response.status_code == 200
...
class TestAPIResponseFormats:
    ...
    def test_sentence_response_format(self, client):
        ...
        response = client.post(
            "/score-sentence", json={"text": "Test zin voor analyse."}
        )
        data = response.json()
    ...
    def test_document_response_format(self, client):
        ...
        response = client.post(
            "/score-document", json={"text": "Eerste zin. Tweede zin. Derde zin."}
        )
        data = response.json()
    ...
    def test_detailed_analysis_response_format(self, client):
        ...
        response = client.post(
            "/analyze-document", json={"text": "Eerste zin. Tweede zin."}
        )
        data = response.json()
        ...
        for sentence in data["sentences"]:
            sentence_fields = [
                ...
                "content_words_proportion",
                "concrete_nouns_proportion",
            ]
