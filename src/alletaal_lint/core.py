...
class WordStats:
    ...
    def is_content_word_excluding_adverbs(self) -> bool:
        ...
        return self.pos in ['NOUN', 'PROPN', 'VERB', 'ADJ', 'NUM', 'SYM'] and self.pos != 'ADV'
...
class LintScorer:
    ...
    @staticmethod
    def calculate_lint_score(freq_log: float, al_max: int, content_words: float, concrete: float) -> float:
        """
        ...
        """
        score = 3.204 + (15.845 * freq_log) - (1.331 * al_max) - (3.829 * content_words) + (13.096 * concrete)
...
class Sentence:
    ...
    def calculate_lint_score(self) -> float:
        ...
        return LintScorer.calculate_lint_score(freq_log, al_max, content_words, concrete)
...
class Document:
    ...
    def get_detailed_analysis(self) -> dict:
        ...
        return {
            ...
            'average_sentence_length': sum(len(s.text.split()) for s in self.sentences) / len(self.sentences) if self.sentences else 0
        }