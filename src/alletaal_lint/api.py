...
class SentenceAnalysis(BaseModel):
    ...
    content_words_proportion: float = Field(
        ..., description="Proportion of content words"
    )
    concrete_nouns_proportion: float = Field(
        ..., description="Proportion of concrete nouns"
    )
...
class DetailedDocumentResponse(BaseModel):
    ...
    sentences: List[SentenceAnalysis] = Field(
        ..., description="Individual sentence analyses"
    )
...
def create_app() -> FastAPI:
    ...
    async def score_sentence(input_data: TextInput) -> SentenceScoreResponse:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error scoring sentence: {str(e)}"
            )
    ...
    async def score_document(input_data: TextInput) -> DocumentScoreResponse:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error scoring document: {str(e)}"
            )
